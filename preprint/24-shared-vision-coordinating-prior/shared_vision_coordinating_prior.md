...
The constructive corollary, established in §6, is that the only architecture satisfying both freeze-freedom and stability preserves per-witness receipts (no scalar aggregation) *and* maintains witness inclusion independent of prior-alignment (no alignment-correlated curation). Either condition alone is insufficient: receipt-lineage without open witnesses is defeated by curation; open witnesses without receipt-lineage are defeated by aggregation.
...
**Internal interpretation (used in §3.4).** When alias-compatibility is at issue, each agent has an additional internal interpretation parameter $\varphi_i \in \mathbb{R}$ such that the agent's effective target is $V_t + \varphi_i$ rather than $V_t$: $a^i_t = -k_i(y^i_t - (V_t + \varphi_i))$. The public alignment layer observes only action signatures, not the $\varphi_i$.
...
### 3.3 Proposition 1: No-Scalar-Free-Lunch (probe-backed conjecture)
...
**Proposition 1 (No-Scalar-Free-Lunch, probe-backed).** *There exists no scalar aggregator $\Phi : \mathbb{R}^* \to \mathbb{R}$ that is simultaneously:*
...
We mark this as a proposition rather than a theorem because the statement at present rests on the empirical aggregation-boundary probe of §4.1 across four canonical aggregator classes (mean, median, max-absolute-value, variance-gated), not on a class-wide impossibility proof. The probe shows that within the four-class sample, every aggregator satisfies (P1) ↔ violates (P2). A formal proof would need to characterize the class of scalar aggregators precisely and show the trade-off is structural rather than a property of the probed sample. We expect the proposition to hold in a precise form once the aggregator class is fixed; this is flagged as an open problem in §8.

If Proposition 1 holds, the constructive consequence is that the only escape from both failure modes is to abandon scalar aggregation entirely — preserving per-witness reports for downstream interpretation rather than collapsing to a summary. This is developed in §6.

### 3.4 Proposition 2: Organizational Alias-Compatibility

Augment the model with per-agent internal interpretation $\varphi_i$ as in §2, so $a^i_t = -k_i(y^i_t - (V_t + \varphi_i))$. Let the public-alignment observable be the action divergence $D(a^1_t, \dots, a^n_t)$ — for concreteness, the cross-agent variance of the action vector at time $t$.

**Proposition 2 (Organizational Alias-Compatibility, scope-conditional).** *Suppose (B1) $V_t \equiv V$ over the observation window and (B2) the closed loop is at equilibrium with equal gains $k_i \equiv k$. Then:*

*(2.a, identifiability collapse under stationary $V$) The equilibrium action vector satisfies $a^i_* = -k\big(x_* - V - (\varphi_i - b_i)\big)$, so the cross-agent variance of $a^i_*$ is determined by the spread of $(\varphi_i - b_i)$ and is invariant to the decomposition into $b$ and $\varphi$ components. When biases $b$ are not independently observable, $D$ under stationary $V$ cannot distinguish a compositionally-aligned cohort (small $\mathrm{Var}(\varphi)$) from a compositionally-divergent one (large $\mathrm{Var}(\varphi)$) with matching $(\varphi - b)$-spread.*

*(2.b, separation under shift) Under a strategic shift $V \to V + \Delta V$ holding $b$ and $\varphi$ fixed, the equilibrium action vector responds via $\delta x_* / \delta V$ uniformly across agents (since $b$ enters only the observation channel and $\varphi$ only the target channel), and the post-shift $D$ surfaces components of the previously-aliased $(\varphi - b)$ spread that the stationary observable could not separate. The §4.2 probe demonstrates the qualitative pattern in a $b \equiv 0$ two-team setting where pre-shock $D \approx 0$ and post-shock $D$ scales with $\varphi \cdot |\Delta V|$.*

*Proof sketch (part 2.a).* At equilibrium with equal gains, the closed-loop balance $\sum_i a^i_* = 0$ (or any analogous fixed-point condition under stable plant dynamics) yields $x_* = V + \tfrac{1}{n}\sum_i(\varphi_i - b_i) = V + \overline{(\varphi - b)}$. Substituting into the action expression: $a^i_* = -k(x_* + b_i - V - \varphi_i) = -k\big(\overline{(\varphi - b)} - (\varphi_i - b_i)\big) = k\big((\varphi_i - b_i) - \overline{(\varphi - b)}\big)$. Cross-agent variance is $k^2 \cdot \mathrm{Var}(\varphi - b)$, which depends on $(\varphi, b)$ only through $\varphi - b$. ∎

*Scope flag.* Part 2.a is exact under (B1)–(B2) with equal gains; the heterogeneous-gain case is more subtle (the equilibrium balance carries gain weights) and is left open. Part 2.b is the qualitative shift-separation observation; the precise functional form of post-shift $D$ depends on the closed-loop dynamics and is demonstrated empirically in §4.2 rather than stated as a closed-form scaling. **We mark this as a proposition rather than a theorem because the qualitative claim is exact in the equal-gain regime but the general statement (heterogeneous gains, nonlinear plants, full quantitative shift-response) is probe-backed and would require a precise dynamical class to support a closed-form theorem.**

**Consequence.** Public alignment to $V$ is observationally alias-compatible: the alignment equivalence class $\{(\varphi, V, b) : (\varphi - b) \text{ spread fixed}\}$ contains both compositionally-aligned and compositionally-divergent decompositions, distinguishable only when $b$ is independently identified or by stress events that shift $V$. This is structurally parallel to the controller-layer observational-masking result of [1, §3.3 case (i)], lifted one scale up.
...
**Consequence.** The gap $|\Delta|$ cannot be closed by internal dynamics: the per-step update is bounded above by $\eta \tau$, and the gap is bounded below by $|\Delta| > \tau$, so closing the gap would require at least $|\Delta|/(\eta \tau) > 1/\eta$ corrective steps in a regime where each step's expected contribution is at most $\eta \tau$. The result holds **regardless of the choice of aggregator $\Phi$** within the 1-Lipschitz class. In particular, shape-preserving receipt-lineage architectures (which fall outside the scalar class but satisfy a generalized version of (C3)) inherit the same bound when applied to the filtered population: the corrective signal has been removed before it reaches the aggregation layer.

**Corollary (necessary-but-not-sufficient architecture).** Shape-preserving aggregation (the constructive class identified by Proposition 1) is *necessary but not sufficient* for reality-tracking governance of a shared prior. The sufficient condition requires both:
...
### 4.1 Aggregation-boundary probe (Proposition 1)
...
The pattern is consistent with Proposition 1: the four aggregators partition into first-moment (mean, median) and shape-sensitive (max-absolute-value, variance-gated) classes, with the first class freeze-locked on bias-split and the second class runaway on asymmetric populations. No tested aggregator achieves both freeze-freedom and stability.
...
The probe produces action divergence of algebraically zero pre-shock regardless of $\varphi$ (within numerical precision), and divergence proportional to $\varphi \cdot \Delta V$ post-shock. The hidden compositional spread is structurally invisible to action-level alignment metrics under stationary $V$ and surfaces under strategic shift, exactly as Proposition 2 predicts for the $b \equiv 0$ case where the $(\varphi - b)$-aliasing reduces to $\varphi$-aliasing alone.
...
Social-choice theory — Arrow's impossibility theorem [5], Gibbard-Satterthwaite [6, 7], Condorcet paradoxes — is closer in flavor. Arrow proves that no aggregation rule over preference orderings satisfies a small axiom set; Proposition 1 has a structurally similar form (impossibility across a family of aggregators) but operates on a different object. Arrow aggregates preferences (strategic, ordinal, action-shaping); Proposition 1 aggregates error signals (descriptive, cardinal, feedback-shaping). The social-choice results address the *design* of one-shot aggregation rules under voter-behavior axioms; Proposition 1 (if it holds in the conjectured form) addresses the *closed-loop dynamic consequence* of repeatedly applying a defective aggregator over time.
...
Federated learning with shared priors or regularization [8] is the nearest technical cousin. A central server aggregates gradient updates from distributed clients with heterogeneous data; the shared prior is the global model, the aggregator is FedAvg or a robust variant. Bias-split is a real open concern in this literature under "heterogeneous clients" or "non-IID data." The structural difference is that federated learning frames this as an *optimization* problem to be solved by a better aggregator, regularizer, or client-reweighting scheme; Proposition 1 conjectures the problem has no solution within the scalar-aggregator class. Proposed fixes (FedProx, FedNova, personalized FL, clustered FL) are consistent with this conclusion: each escapes mean-aggregation masking only by abandoning the scalar-summary form or the assumption of a single shared prior.
...
The constructive architectural implication of §6 — that escape from the three-layer pathology requires preserving per-witness receipts and maintaining open witness inclusion — connects to ongoing work on governance architectures for AI agent systems where receipt-based authority and per-action lineage are proposed as first-class primitives [2]. The contribution of this paper to that line is to identify the formal problem such architectures solve: receipt-lineage is the architectural answer Theorem 1 and Proposition 1 jointly motivate (with the strength of "motivate" depending on whether Proposition 1 is taken in its probe-backed or theorem-strength form). Reciprocally, Theorem 3 identifies what receipt-lineage architectures *cannot* solve on their own: open witness inclusion must also be maintained, or the receipts produce no corrective signal.

## 8. Open / deferred

- **Proposition 1 as a theorem.** The strongest item on the list. A proof requires fixing the scalar-aggregator class precisely (continuous? Lipschitz? permutation-invariant?) and exhibiting the structural trade-off between (P1) and (P2) within that class. We expect the proposition to hold in a precise form and treat its theorem-ization as the most valuable single next piece of work.
...
[1] Beck, J. (2026a). *Ops Is Control with a Non-Self-Identical Controller.* Δt Framework, Paper 23. Preprint.
