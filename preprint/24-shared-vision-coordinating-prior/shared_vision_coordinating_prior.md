...
**Status:** Preprint v0.2 (revised 2026-04-27; not yet pushed to Zenodo)
...
Agents agree on the public level at $V_0$ and disagree only on the local continuation rule — what the token *means* under movement. The control law becomes $a^i_t = -k_i(y^i_t - A_i(V_t))$. The public alignment layer observes only action signatures, not the $\varphi_i$. The earlier additive form $V_t + \varphi_i$ is recovered as the special linear case $A(V) = V$ with $A_i(V) = V + \varphi_i$ — but that case introduces nonzero pre-shock action divergence under stationary $V$ and is therefore set aside in favor of the local-gradient form, which captures the "agree on level, disagree on direction" structure that the §3.4 alias-compatibility argument requires.
...
**Corollary (necessary-but-not-sufficient architecture).** Shape-preserving aggregation (the constructive class identified by Conjecture 1) is *necessary but not sufficient* for reality-tracking governance of a shared prior. The sufficient condition requires both:
