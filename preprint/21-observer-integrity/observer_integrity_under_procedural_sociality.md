...
**Observer integrity is not uniform across model families.** Two models (Qwen3 8B and 14B) showed zero perturbation under identical conditions where three others showed strong contamination. This was not a function of model size: Qwen3 8B (8 billion parameters, zero perturbation) and Llama 3 (8 billion parameters, strong perturbation) are the same parameter count but different architectures. The perturbation susceptibility is architectural, not parametric.
...
The central empirical question — continuous or regime-switching? — has a preliminary answer: continuous in most models tested, but with amplitude that varies by architecture. The boundedness that makes subtraction feasible is not a property of the assay alone. It is a joint property of assay design and observer architecture, and cross-model triangulation is not an optional refinement but a prerequisite for trusting any single-model result.
