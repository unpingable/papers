...
This is an explicitly human-managed directory. It is allowed to be messy in ways that don't always cleanly reconcile — *working* is in the name. Several notes carry their own status / stance / falsifier / pressure-evidence markers; older notes may call the last of these a *forcing case*. That historical label is not an operative consumer gate. The discipline is name-early-ratify-lazily, not pre-publication coherence. Most notes flag whether they are paper-shaped, recognition-only, scaffold, or candidate. Promotion to the numbered preprint series happens elsewhere and is not implied by anything here.

**Formal-work policy (2026-07-14):** Formalization may precede and lead implementation. A runtime consumer never grants permission to develop a theorem. Formal admission is governed by theorem shape, semantics, overlap, proof, and anti-vacuity controls; public promotion is a separate custody and compatibility decision. An implementation that cites a formal artifact has named its intended contract, not proved conformance—conformance requires an explicit mapping plus runtime evidence or a refinement argument.
...
- **adversarial-witness-protocol.md** — *Adversarial Witness Protocol v0.1*: Protocol-shaped working note framed as the **observational dual of the admissibility kernel** — where the kernel defines conditions under which a binding may be admissible, AWP defines observable contradiction patterns indicating that declared admissibility has decoupled from operational practice. Six contradiction families (decorative appeal, override groove, asymmetric friction, receipt theater, scope creep, vocabulary convergence) map one-to-one onto existing primitives (FiatAdmissibility, laundering-move-watchlist, non-reciprocal-admissibility-flow, PublicReceiptRefinement, compression-becomes-authority). No-aggregation doctrine promoted from UI rule: *roll-up converts witness into certifier*. Tombstone clause = pre-committed voluntary admissibility revocation, published before incentives to obscure exist. Two candidate handles surfaced not promoted: *advance tombstone* (voluntary revocation, sibling to admissibility-decay family) and *aggregation-as-authority-laundering* (specific laundering-move subset). §0 motive-claim guardrail explicit: contradictions are evidence of declared-observed gap, not attribution of cause (capture / overload / temporal-basis decay / implementation lag all surface as the same family). Anti-build guardrail explicit: the suspiciously buildable repo skeleton is evidence of implementability, not permission to operationalize. Methodological cross-link at Section 7. Keeper line: *a witness may produce exhibits; it may not become the authority that its exhibits are supposed to constrain.* Provenance: cross-model session (ChatGPT + claude-web + claude-code), 2026-05-19.

- **admissibility-trajectory-audit.md** — *Admissibility-Trajectory Audit*: Scout / audit-grounded / not primitive-promoted (added 2026-05-14). Records the residue from an extended ChatGPT + web-Claude session on "closure standing" as a candidate training-signal / witness-format for deployed tool-using model systems. Most proposed deltas collapse into existing substrate (FiatAdmissibility / SurfaceAuthorization for the authority gap; Stale Binding for closed-then-stale; receipt-vs-authority Lean split for evidence/commitment dissociation; Binding Reconciliation precedent for the witness ledger). Three residues survive: the quartet diagnostic line (evidence / consequence / feedback / permission stop living in the same agent), the blocker-not-missing-belief prose lemma, and a withdrawn-vs-stale asymmetry. The narrow test (§ 4) finds withdrawn-before-binding is *not* a failure mode but a successful refusal to bind — non-failure / explanatory contrast under Temporal Basis Decay, not a new primitive. Keeper line: *a stale binding has inherited obligations; a withdrawn provisional has none.* Explicit non-promotion section enumerates what was *not* filed (no `ClosureStanding.lean`, no paper dir, no witness-format spec, no primitive entry) and the three re-open triggers.
...
- **nondegenerate-store-semantics.md** — *Nondegenerate Store Semantics*: Identifies the three independent commitments (nontrivial store effects / verdict-sensitive derivation / non-pre-blockage by corrective steps) that would have to be true for the recorded investigative null `corrective_then_forward_is_not_monotone` to become decidable. Names the smuggling risks each commitment introduces. The note does not itself supply a non-vacuous theorem statement or justify new axioms; later formal work stands on its own intrinsic review and may precede implementation.
...
- **parked-leads.md** — *Parked Leads*: Register filed 2026-05-20 from the eight-slice brainstorm + audit. Records six candidate leads (Coverage Cannot Mint Absence / Receipt Envelope MVS / Serialized Authority Becomes Claim / Cartographer Aperture / Narrative Cannot Mint Authority / Specification Does Not Mint Object) that the audit refused to authorize as new construction. Its original reopening predicates included a *live repo consumer*; under the current policy that is correspondence evidence, not permission to formalize. Reopening now turns on a coherent non-redundant theorem shape, new negative fixture, cross-tool recurrence, or a contradiction not covered by the existing kernel. Three buckets — warm (NQ/Wicket/AG/Nightshift supply useful pressure), cold (Cartographer Aperture — defer public promotion until two non-chat specimens), doctrine/methodology (essay/keeper-line layer, not kernel). Composes with [gate stack](../../.claude/projects/-home-jbeck-git-papers/memory/feedback-gate-stack.md) — every lead lives at *handle* stage. Load-bearing line: *"Not earned yet" is not "false." It is "don't let the name spend authority."* Anti-goblin-door policy compressed; refuses backlog-converts-to-obligation drift.

- **rogue-references.md** — *Rogue Reference Notes: Nyquist and Ellul*: Method-coordinate note (not lineage claim) naming two triangulation handles. Nyquist as the inverse posture — *reading aliasing as testimony*, inferring failed sampling/control boundaries from the distortion artifacts (stale dashboards, demos-as-theory, cached-as-fresh-authority, technique-aliasing in AI metrics, etc.). Ellul as the institutional dimension: technique expands and reinforces itself until the regulator gets *confused with virtue*. Carries Chris's compression — *if it cannot be measured, management replaces it with something measurable* — as the Ellul move in miniature. Anti-self-inflation guardrail explicit: coordinate axes, not ancestry; no résumé-core, no manifesto-in-a-fake-mustache. Working note, not lineage claim.

- **adversarial-witness-protocol.md** — *(primary entry at Section 2)*: Methodological cross-link. Carries the no-aggregation doctrine (*roll-up converts witness into certifier*), capture-tripwire checklist, and tombstone-as-revocation discipline. Methodological siblings: *methodology-as-operational-discipline*, *laundering-patterns-reliability-practice*.
...
- **post-sre-ops.md** — *Post-SRE Ops — Operational Admissibility*: Positive-thesis sibling to *sre-as-shock-absorption*. Working note / essay seed filed 2026-05-20; manifesto-risk flagged high. **Core thesis:** SRE made reliability measurable; post-SRE ops makes operational claims admissible. The post-SRE question is *"what claim is this system allowed to make, from which witness, under what coverage, with what consequence, and what must it refuse to say?"* — the failure mode is no longer absence of telemetry but *overclaim from partial observability*, made operational by AI agents converting plausible-sounding claims into executable workflows. Method shape: *observe → constrain claim → test refusal → preserve receipt → update doctrine.* Sharper-version keeper: *"scientific method, except the specimen is not the system; the specimen is the claim the system tried to make."* Seven-refusal compression (*observation is not testimony; testimony is not authority; success is not authorization; silence is not health; green is not recovered; receipt is not validity; automation is not governance*), each backed by a specific kernel module or working note. Tool constellation mapped: NQ = claim preflight, AG = admissibility-bound action authorization, Wicket = admissibility gate for authority-bearing changes, WLP = eventual receipt grammar / wire protocol substrate, Nightshift = preserved-uncertainty coordination. Anti-manifesto guardrails explicit. Book-claude branch carries the public-facing thesis variant; this file is the internal methodology compression.
