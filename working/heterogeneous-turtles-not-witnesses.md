...
**Filed:** 2026-06-06 (Fri PM). **Status:** doctrine note + agentic-system specimen bridge; **2026-06-09 resolved as prose doctrine** — inherits parent ([`admissibility-vs-arms-control.md`](admissibility-vs-arms-control.md) § 2026-06-09 status). **Policy correction, 2026-07-14:** the carrier mismatch is a formal design constraint, not a consumer gate. A focused receipt-production-type model may lead later code; public promotion and mutation of the current carrier remain separate. Original 2026-06-06 curdling timer closed 2026-06-07.
...
- **Not represented by the current Lean carrier.** `RefusalPropagation.lean` supports only binary/topological refusal; the receipt-production-type "different type" predicate is absent. A bounded Scratch model may represent it before a runtime consumer exists, but it must resolve the type taxonomy and prove a discriminating result. The carrier-model `evidence_ref` field only gestures at the distinction and is not itself a discharge.
...
**Lean status:** the existing formal carrier (`RefusalPropagation.lean`) supports only binary/topological refusal. This doctrine names the *receipt-production-type* axis (what production mechanism minted the witness — model vs compiler vs sensor vs ledger vs human observation vs reproducible experiment vs policy engine), which the current carrier does not represent. The documented `State := observable | refused` decision is a real compatibility boundary, confirmed load-bearing by the 2026-06-09 CascadeSound runtime audit, but it is not a runtime-consumer prerequisite for formalization. A probe may lead by choosing a precise receipt-type model with non-vacuous controls; changing the public carrier is a separate custody decision.
