...
Lean-shaped when:               (the intrinsic theorem / type boundary)
Promotion evidence:             (evidence relevant to public custody, if any)
```
...
This is a **doctrine-promotion gate**, not a theorem-admission gate. Formal work may begin earlier when a coherent, non-redundant statement can be tested. Recurrence affects confidence and public status; a consumer never grants permission to develop the theorem.

## Lean-worthy gate

A move earns a Lean encoding only when:

1. **The boundary has an intrinsic theorem or type-refusal shape.** The formal statement must identify the prohibited lift without depending on a particular runtime function already making the mistake. A live failure can sharpen the statement, but is not a prerequisite.
2. **The classification table is sharp enough to encode and test non-vacuously.** Require meaningful positive and negative controls; if the categories are still drifting in prose, Lean will only solidify the drift.
...
Formal existence, public promotion, and runtime conformance remain separate. A proved file may lead code without immediately joining the public surface. Conversely, an implementation's citation of a theorem names the intended contract but does not establish conformance; that claim needs an explicit mapping plus runtime evidence or a refinement argument.
