...
**Posture:** Phrase mine + one named gap, **consumer-watching regime** (updated 2026-05-19, same day). The initial audit confirmed no live consumer; same-day operator update flagged `~/git/nq-witness` (claim-preflight registry, profile-defined claim-kinds, witness-standing vocabulary) as candidate consumer with DNS potentially becoming the second claim-kind. The regime has *not* flipped to consumer-backed — per audit-procedure §2½, that requires the consumer to *implement the shape*, not to *name it as candidate*. Watching, not building.
...
| Negative caching | *A cached denial is still a claim.* / *Absence may be admissible, but only under signed/delegated scope and TTL.* | `Authority.BasisVerdict.noBasis` collapses two shapes; see "real gap" below | **Real gap, prose-only residue + forcing-case spec** |
...
**Forcing-case spec:** a consumer surfaces requiring distinction between *"I have no basis"* and *"I have a signed, scoped, TTL-bounded claim of absence."* The candidate filing would be a small additional `BasisVerdict` variant or a sibling `AuthoritativeAbsence.lean` module — deflationary name, refuses the Δt-universal-solvent framing.
...
Regime classification updates: gap moves from *"1 specimen, no consumer"* to *"1 specimen, consumer surfacing — watching, not flipped."* Per audit-procedure §2½, a *live consumer that already implements the shape* authorizes a small axis-kernel; a *named candidate consumer* does not. The Standing/Freshness precedent required Standing's production-quality Rust implementation, not Standing's name. Promotion stays gated on concrete implementation of DNS-as-claim-kind in `nq-witness`, not on naming.

Until concrete implementation: **tracked, watching, not promoted.** Specimen count remains 1; consumer regime is *watching*.
...
- A new Lean module (`DnsAdmissibility.lean`, `BailiwickStanding.lean`, `Glue.lean`, `AuthoritativeAbsence.lean`, anything DNS-shaped).
...
- **For a Lean module:** a constellation-resident consumer (per audit-procedure §2½ consumer-check). DNS-the-protocol is not a repo-mounted consumer.
...
- **claude-code-papers (this writeup):** executed the kernel-overlap audit procedure (anti-collapse-runbook §2½), confirmed no constellation consumer at audit time, filed as parking-lot working note. No module authorized. No primitive promoted. One real gap (authoritative absence) tracked with explicit forcing-case spec. Four STRONG residues named with filing targets but not patched into target files.
- **Same-day operator update (2026-05-19):** `nq-witness` flagged as candidate consumer with DNS as potential second claim-kind. Regime classification updates from *no consumer* to *watching*; the gate remains gated on concrete implementation, not on conversational naming. Aperture note added: NQ-design content elides to nq-claude per cross-SME routing discipline; this note records the surfacing without elaborating on the mapping.
