...
**Filed:** 2026-06-06 (Fri). **Updated:** 2026-07-14 after the papers-fragment extraction and formalization-first policy correction. **Status:** custody ledger, not an authorization queue. **Source:** the 2026-06-06 custody audit + cleanup pass, followed by the 2026-07-14 extraction audit.

This file is the single retrievable answer to *"where is the Lean?"* for the work captured in this corpus at the dates above. It is NOT doctrine and NOT a consumer-held build queue; it is an after-action custody snapshot. Formalization may lead implementation. Update when custody states change; do not infer runtime conformance or public authority from Scratch contact.
...
- `~/git/lean/LeanProofs/Scratch/ControlPathIndependence.lean` — checked Scratch core extracted from the CPI candidate. Carries reachability, architecture-independence, wiring⇛standing/coupling non-collapse, and `InterlockBasis`; the four vocabulary-deficient failure predicates remain prose debt. Formalization does not wait on NQ / Wicket / Governor. Not imported; not doctrine; not runtime conformance.
- `~/git/lean/LeanProofs/Scratch/UncertaintyCustody.lean` — checked Scratch extraction. Closes the two finite UC/RC independence countermodels that the Markdown source left as `sorry`. Not imported; promotion needs scope/overlap/custody review, not a consumer.
- `~/git/lean/LeanProofs/Scratch/CommitmentStanding.lean` — checked Scratch correction of the source note's false eventual-revocation theorem. Keeps true subset/nonstanding laws, a fixed-declaration nondetermination result, and a constant-sequence countermodel to inevitability. Not imported; no historical or universal liveness claim.
- `~/git/lean/LeanProofs/Scratch/SignalAuthority.lean` — checked formalization-leading no-objection/solicitation slice. Exact group/decision/window binding, solicitation and ledger coverage, positive and collapse specimens. Not imported; does not prove consent or runtime conformance.
- `~/git/lean/LeanProofs/Scratch/StatusConversionBinding.lean` — checked repair of the field-guide status-conversion sketch. Exact source/target/report/time indices plus external issuer-standing, scope, and conversion-permission relations. Not imported.
- `~/git/lean/LeanProofs/Scratch/AffectiveCouplingClassification.lean` — checked classification-semantics slice. Uses a declared threshold, avoids causal `capture`/`toxic` certification, and proves valence does not determine the signature. Not imported; runtime measurement remains external.
- `~/git/lean/LeanProofs/Scratch/ConsolidationController.lean` — checked extraction of the Schmitt-trigger supervisor. Repairs the source sketch and closes conditional mode-specific B boundedness. No K/X/R dynamics, liveness, eventual-release, or runtime claim. Not imported.
- `~/git/lean/LeanProofs/Scratch/NoSilentProjection.lean` — checked theorem-only extension over the resident obligation lattice. Proves Lift cannot silently discharge Projection and proves both directions of the Deform/Exception separation. Not imported; no parallel family directory.
...
## Field-guide examples (source sketches superseded by Scratch)

- **`NoObjectionRequiresSolicitation`** — the Markdown block remains a field-guide/source sketch; its repaired formal descendant is checked `Scratch/SignalAuthority.lean`. The Scratch file does not make the field-guide demon a separate kernel or ratify the doctrine.
- **`ReportedStatusRequiresConversionWitness`** — the Markdown block is superseded by checked `Scratch/StatusConversionBinding.lean`, which makes the source fields proof-bearing and externally governed. Same custody fence.
...
- **`NoSilentDelegation`** — still prose-only; any formal artifact needs a distinct non-duplicative theorem shape.
- **`NoSilentException` / `NoSilentProjection`** — the relevant non-collapse theorems now live together in checked `Scratch/NoSilentProjection.lean`, reusing the resident obligation table. No consumer prerequisite and no automatic promotion.
...
- `extracted-to:<path>` — block was extracted; the checked source lives at the given path. Its own header declares whether that source is Scratch, annex, or public/imported; extraction alone is not promotion.
- `superseded-by:<path>` — block was extracted and has since diverged; the path is canonical, the markdown is stale.
- `frontier-do-not-extract` — block is doctrine-shape or otherwise not yet a coherent, non-duplicative proposition. The hold must name an intrinsic theorem/scope/overlap reason; consumer absence is not a valid reason.
...
- [`../bridge-obligation-lattice.md`](../bridge-obligation-lattice.md) — Frontier-family doctrinal home and status map; `NoSilentProjection` now points to checked Scratch while other families may remain prose-only.
