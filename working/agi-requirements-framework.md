...
**Admissibility frame:** Trace inspection is necessary but not sufficient. The trace itself can be a *captured report* — internally legitimate, locally inspectable, while the loop it sits inside has been redirected. A reasoning trace generated by a system whose sensing, estimation, objective, admissibility, or reporting layer has been adversarially redirected will look *more* coherent, not less, because it is generated to be inspectable. The trace can preserve internal legitimacy while defended value decays. (See `working/loop-capture.md` on $L_t / V_t$ divergence; surface 5: reporting capture; *Witness Necessity Claim* — detection requires at least one channel not co-captured by the same transformation.)
...
**Admissibility-layer companion:** A worked example of admissibility-layer formalization exists in the sibling Lean repository under `LeanProofs/Admissibility/` — Authority, StateTransition, Derivation, Execution, Corrective, and WitnessInvariance modules. Not safety-property verification in the §2.7 sense; the same formal-verification discipline applied to the admissibility layer underneath. The two are complementary — admissibility verifies *whether actions are authorized to bind*; safety verification verifies *whether authorized actions are harmful*. Loop Capture's $L_t / V_t$ divergence makes the gap explicit: an action can be admissible-internally-valid while degrading defended value. Bridging the two layers is open work (see *Reverse-Gap Notes* below if added; for now, named as future work).
