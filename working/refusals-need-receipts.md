...
**Filed:** 2026-06-06 (Fri PM). **Status:** doctrine note + sprint manifest. **Sprint #1 landed green 2026-06-06 PM** — `~/git/lean/LeanProofs/Admissibility/Mandamus.lean` is scratch-checked with the three boring theorems (`sufficient_ministerial_before_horizon_implies_owes`, `nondecision_past_horizon_implies_deemed_refusal`, `blocking_predicate_does_not_reset_matter_clock`) after the four corrections below settled (progress not acyclic / claimant-capability / blocking-predicate discipline / record-not-compulsion) plus the fifth (ministerial/discretionary in types) plus the operator's catch on the unbound `Receipt` parameter ("the framework bit its own author"). **Sprint #2 deferred** to `~/git/lean/LeanProofs/Admissibility/Mandamus.lean` § Sprint #2 list and to the file body below. **Curdling discipline** original framing — *do not formalize before 2026-06-07; the four corrections must settle first* — held in the sense that the build went only after the corrections landed and the operator caught the framework's own forged-witness slot. Status-line patched 2026-06-06 PM under Table-Flip Override condition #7 (audit found contradiction between this file and the import graph + ledger).
