...
- The primitive describes a failure shape, not yet a remedy architecture. The mitigation laws above are operational hygiene, not formal guarantees. A formal version — analogous to corrective monotonicity's `RecoveryEnv` gate — could type tasks against observation freshness and refuse to plan against unrefreshed task state. A bounded model may establish that contract before a runtime forcing case; it must still supply a precise task/observation relation, a non-vacuous refusal theorem, and proof controls. Runtime enforcement is separate.
