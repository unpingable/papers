...
**Lean formalization:** `~/git/lean/LeanProofs/Admissibility/WitnessInvariance.lean` — relational form (`Encapsulated` / `MovesUnderExcludedPerturbation`) plus typed perturbation-relation refinement (`EncapsulatedWrt` / `MovesUnderDisturbance` parameterized by an *allowed-perturbation relation* on the disturbance class, per chatty 2026-05-08), `moves_under_disturbance_implies_not_encapsulated_wrt` boundary theorem, `encapsulated_wrt_mono` refinement-monotonicity corollary, `encapsulated_wrt_iff_relational` bridge, and toy counterexample showing selectivity ↛ encapsulation.
...
4. **Regime leakage.** The witness is invariant under the declared perturbation class only inside operating regime `Γ`. Outside `Γ`, invariance fails. Most real witnesses are *regime-bounded*, not globally qualified — the regime layer is the next refinement after the disturbance layer. Not yet formalized in the Lean module; flagged as next gate.
...
- **Multi-model lineage:**
  ...
  - chatty (2026-05-08 sharpening pass — DD/ICP/WIF mode-distinction; *disturbance class is a perturbation relation, not a type*; refinement-monotonicity and bridge theorem; four-part witness-failure classification including the unformalized regime-leakage layer).
  - claude-code-papers (perturbation-relation Lean refinement; this note's update pass).
